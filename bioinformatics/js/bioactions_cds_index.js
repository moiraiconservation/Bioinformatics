...
Bioaction.prototype.activate_cds_index = function() {
  ...
  .then(() => {
    if (this.ncbi_record.cds_from_genomic_url) {
      this.get_cds_record()
      .then(() => {
        if (this.cds_record.percent_uploaded >= 100) {
          this.get_cds_index_record()
          .then(() => {
            this.actions.cds_index.status = "loading";
            this.update();
          }); // end then get_cds_index_record()
        } // end if (this.cds_record.percent_uploaded >= 100)
      }); // end then get_cds_record()
    } // end if (this.ncbi_record.cds_url)
...
Bioaction.prototype.create_cds_index = function(options) {
  ...
  if (options.callback) { this.actions.cds_index.callback = options.callback; }
  if (options.callback_arguments && options.callback_arguments.length) { this.actions.cds_index.callback_arguments = options.callback_arguments; }
  if (options.initial_status) { this.actions.cds_index.status = options.initial_status; }
  if (!options.skin) { options.skin = this.default_skin; }
  const action = options.skin(this.actions.cds_index.id, this);
  this.actions.cds_index.skin = action;
  ////////////////////////////////////////////////////////////////////////
  ...
  if (options.element_id) {
    ...
    this.html_element.cds_index = document.getElementById(options.element_id);
    this.html_element.cds_index.style.display = "block";
  } // end if
  ...
  action.button.addEventListener("click", function() {
    action.button.blur();
    this.cds_index_record.metadata.delta_second = 1;
    ...
    try {
      ...
      metaObj.num_records = this.cds_index_record.num_records ? this.cds_index_record.num_records : 0;
          ...
          if (responseText) {
            ...
            .then(responseText => {
              ...
              if (responseText) {
                ...
                let job = { status: 'index', command: 'create', organism_name: this.organism_name, data: data, num_uploaded: this.cds_index_record.num_uploaded };
                ...
                bioWorker.onmessage = function(e) {
                  switch(e.data.status) {
                    ...
                    case 'step2': {
                      this.cds_index_record.num_records = e.data.work;
                      metaObj.num_records = this.cds_index_record.num_records;
                      update_metadata(metaObj);
                      reset_progress_bar(this.cds_index_record.num_records);
                      show_progress_bar();
                      progress_bar_text("Step 2 of 2: Saving records");
                      update_progress_bar(this.cds_index_record.num_uploaded);
                      break;
                    } // end case
                    case 'step3': {
                      update_progress_bar(e.data.work.chunk_size);
                      progress_bar_subtitle("Record " + e.data.work.index + " of " + this.cds_index_record.num_records);
                      this.cds_index_record.num_uploaded = this.cds_index_record.num_uploaded + e.data.work.chunk_size;
...
///////////////////////////////////////////////////////////////////////////////
// PROTOTYPE //////////////////////////////////////////////////////////////////
Bioaction.prototype.create_cross_species_cds_map = function(options) {
  if (typeof(options) === "undefined") { options = new BioactionOptions; }
  if (options.element_id) {
    let element_check = document.getElementById(options.element_id);
    if (element_check) {
  ...
  ////////////////////////////////////////////////////////////////////////
  // REGISTER VARIABLES WITH PARENT OBJECT ///////////////////////////////
  if (options.callback) { this.actions.cross_species_cds_map.callback = options.callback; }
  if (options.callback_arguments && options.callback_arguments.length) { this.actions.cross_species_cds_map.callback_arguments = options.callback_arguments; }
  if (options.initial_status) { this.actions.cross_species_cds_map.status = options.initial_status; }
  if (!options.skin) { options.skin = this.default_skin; }
  const action = options.skin(this.actions.cross_species_cds_map.id, this);
  this.actions.cross_species_cds_map.skin = action;
  ////////////////////////////////////////////////////////////////////////
  // INITIALIZE SKIN ELEMENTS ////////////////////////////////////////////
  ...
  option_area += '<input type="checkbox" id="slider-checkbox-' + this.actions.cross_species_cds_map.id + '-' + this.id + '" value="1">';
  ...
  if (options.element_id) {
    document.getElementById(options.element_id).appendChild(action.area);
    this.html_element.cross_species_cds_map = document.getElementById(options.element_id);
    this.html_element.cross_species_cds_map.style.display = "block";
  } // end if
  else { document.body.appendChild(action.area); }
  $('[data-toggle="tooltip"]').tooltip();
  this.activate_cross_species_cds_map();
  //////////////////////////////////////////////////////////////////////
  // EVENT LISTENERS ///////////////////////////////////////////////////
  action.button.addEventListener("click", function() {
    //////////////////////////////////////////////////////////////////
    ...
    // LOCAL VARIABLES ///////////////////////////////////////////////
    action.button.blur();
    this.get_cross_species_cds_map_record()
    .then(() => {
      this.cross_species_cds_map_record.species1.metadata.delta_second = 1;
      this.cross_species_cds_map_record.species2.metadata.delta_second = 1;
      find_MRCA(this.actions.cross_species_cds_map.species1.organism_name, this.actions.cross_species_cds_map.species2.organism_name)
      .then(mya => {
        create_loading_box("Finding records", true);
        const CDHIT_percent_identity = mrca_to_blosum_number(mya).toString();
        let id1 = this.cross_species_cds_map_record.species1.num_uploaded + 1;
        let id2 = this.cross_species_cds_map_record.species2.num_uploaded + 1;
        let name1 = this.actions.cross_species_cds_map.species1.organism_name.replace(/ /g, '_');
        let name2 = this.actions.cross_species_cds_map.species2.organism_name.replace(/ /g, '_');
        let max_id1 = this.actions.cross_species_cds_map.species1.cds_record.num_records;
        let max_id2 = this.actions.cross_species_cds_map.species2.cds_record.num_records;
        let split_name1 = this.actions.cross_species_cds_map.species1.organism_name.split(' ');
        let split_name2 = this.actions.cross_species_cds_map.species2.organism_name.split(' ');
        ...
        //////////////////////////////////////////////////////////////////
        // FUNCTION OPTIONS //////////////////////////////////////////////
        let cds_index_options = { };
        cds_index_options.cache = 0;
        let element_check = document.getElementById('slider-checkbox-' + this.actions.cross_species_cds_map.id + '-' + this.id);
        if (element_check) { cds_index_options.cache = element_check.checked; }
        //////////////////////////////////////////////////////////////////
        ...
        obj1a.num_records   =   this.cross_species_cds_map_record.species1.num_records;
        ...
        obj2a.num_records   =   this.cross_species_cds_map_record.species2.num_records;
        // file_guard command for saving the map record
        ...
        organism1.mrca              = mya;
        organism1.this              = this.actions.cross_species_cds_map.species1;
        ...
        organism2.mrca              = mya;
        organism2.this              = this.actions.cross_species_cds_map.species2;
        ...
        xspecies_cds_index(organism1, organism2)
        .then(() => { xspecies_cds_index(organism2, organism1); })
        //////////////////////////////////////////////////////////////////
        // METHOD ////////////////////////////////////////////////////////
        function xspecies_cds_index(organism1, organism2) {
          //////////////////////////////////////////////////////////////
          ...
          //      organism1.cds_db-----> select                  //
          //      organism2.index_db--------> select                  //
          //      organism2.cds_db-----> select                  //
          //      organism1.alignment-------> cds_index                  //
          ...
          return new Promise(function(resolve, reject) {
            ...
            let kmers = [];
            let kmer_size = 4;
            organism1.cds_db = new Worker(current_base_url + '/workers/js/client_table.js?version=' + guid());
            organism2.cds_db = new Worker(current_base_url + '/workers/js/client_table.js?version=' + guid());
            ...
            if (cds_index_options.cache) {
              options.seed.filter_low_complexity = false;
              hide_loading_box(true);
              create_loading_box("Loading Proteins", true);
              let job = { command: 'load', database: 'cds_db', table: organism1.name, limit: organism1.this.cds_record.num_records };
              organism1.cds_db.postMessage(job);
            } // end if
            else {
              options.seed.filter_low_complexity = true;
              hide_loading_box(true);
              create_loading_box("Connecting to Databases", true);
              job = { command: 'connect', database: 'cds_db', table: organism1.name };
              organism1.cds_db.postMessage(job);
            } // end else
            //////////////////////////////////////////////////////////
            // ORGANISM1 cds_DB MESSAGES ////////////////////////
            organism1.cds_db.onmessage = function(e) {
              switch(e.data.status) {
                case 'complete': {
                  switch(e.data.command) {
                    ...
                    case 'connect': {
                      let job = { command: 'connect', database: 'cds_db', table: organism2.name };
                      organism2.cds_db.postMessage(job);
                      break;
                    } // end case
                    case 'transform': {
                      hide_progress_bar();
                      ...
                      let job = { command: 'load', database: 'cds_db', table: organism2.name, limit:  organism2.this.cds_record.num_records };
                      organism2.cds_db.postMessage(job);
                      break;
                    } // end case
                    case 'select': {
                      ...
                      if (job.where.length) { organism2.index_db.postMessage(job); }
                        ...
                        .then(() => {
                          ...
                          if (organism1.current_id > organism1.max_id) {
                            organism1.cds_db.terminate();
                            organism2.cds_db.terminate();
                    ...
                    case 'structure': {
                      hide_loading_box();
                      create_progress_bar("Masking Low Complexity Regions", true, organism1.this.cds_record.num_records);
                ...
                case 'progress': {
                  switch(e.data.command) {
                    case 'transform': {
                      ...
                      progress_bar_subtitle("Sequence " + amount_masked + " of " + organism1.this.cds_record.num_records);
                      break;
                    } // end case
                    default: { break; }
                  } // end switch
            ...
            //////////////////////////////////////////////////////////
            // ORGANISM2 cds_DB /////////////////////////////////
            organism2.cds_db.onmessage = function(e) {
              switch(e.data.status) {
                case 'complete': {
                  switch(e.data.command) {
                    case 'load': {
                    ...
                    case 'connect': {
                      let job = { command: 'connect', database: 'cds_index_db',  table: organism2.name };
                      organism2.index_db.postMessage(job);
                      break;
                    } // end case
                    case 'select': {
                      options.search_space.num_characters = organism2.this.cds_record.options.num_characters;
                      options.search_space.num_sequences  = organism2.this.cds_record.num_records;
                      let choices = [];
                    ...
                    case 'structure': {
                      ...
                      let job = { command: 'load', database: 'cds_index_db',  table: organism2.name, limit: organism2.this.cds_index_record.num_records };
                      organism2.index_db.postMessage(job);
                      break;
                    } // end case
                    default: { break; }
              ...
              switch(e.data.status) {
                case 'complete': {
                  switch(e.data.command) {
                    ...
                    case 'connect': {
                      hide_loading_box(true);
                      create_progress_bar("Cross-species Mapping", true, organism1.this.cds_record.num_records);
