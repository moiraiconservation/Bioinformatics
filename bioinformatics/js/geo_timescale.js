...
// TIMESCALE //////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//  This is a modified version of the GitHub repository found here:
...
var timescale = (function() {
  var data = { oid: 0, col: "#000000", nam: "Geologic Time (Reset)", children: [] },
      interval_hash = { 0: data },
      currentInterval,
      dragStart, transformStart;
  // Variables added for custom functions
  var markerGroup;
  let markers = [];
  return {
    //////////////////////////////////////////////////////////////////////
    ...
    "init": function(div) {
      ...
      var drag = d3.behavior.drag()
        ...
        .on("drag", function() {
          ...
          d3.select(".timescale").select("g").attr("transform", function() {
      ...
      for(var i=0; i < geo_intervals.records.length; i++) {
        var r = geo_intervals.records[i];
        r.children = [];
        r.pid = r.pid || 0;
      ...
      textGroup.selectAll("fullName")
        ...
        .enter().append("svg:text")
          ...
          .attr("height", function(d) { return y(d.dy); })
          .attr("class", function(d) { return "fullName level" + d.lvl + " noselect"; })
      ...
      textGroup.selectAll("abbrevs")
        ...
        .enter().append("svg:text")
          ...
          .attr("class", function(d) { return "abbr level" + d.lvl + " noselect"; })
      ...
      markerGroup = time.append("g").attr("id", "markerGroup");
    ...
    "goTo": function(d) {
      ...
      d3.selectAll(".marker_text").remove();
    ...
    //////////////////////////////////////////////////////////////////////
    // METHOD ////////////////////////////////////////////////////////////
    // returns the geological intervals that contain the specified date
    "when": function(mya) {
      const when = [];
      for (i = 0; i < geo_intervals.records.length; i++) {
        if ((mya > geo_intervals.records[i].lag) && (mya <= geo_intervals.records[i].eag)) {
          when.push(geo_intervals.records[i]);
        } // end if ((marker > geo_intervalss.records[i].lag) && (marker <= geo_intervalss.records[i].eag))
      } // end for loop
      when.sort(function(a, b) {
    ...
    //////////////////////////////////////////////////////////////////////
    // METHOD ////////////////////////////////////////////////////////////
    "add_marker": function(mya, text, highlight) {
      if (typeof(mya) === "undefined") { return; }
      if (typeof(highlight) === "undefined") { highlight = false; }
      let marker_found = false;
      let place_in_queue = 0;
      let previous_marker_x = -Infinity;
      for (let i = 0; i < markers.length; i++) {
        if ((markers[i].mya === mya) && (markers[i].text === text)) {
          marker_found = true;
          place_in_queue = i;
        } // end if
      } // end for loop
      if (!marker_found) {
        place_in_queue = markers.length;
        markers.push({ mya: mya, text: text, highlight: highlight });
        markers.sort(function(a, b) {
          if (a.mya > b.mya) { return -1; }
          else { return 1; }
        }); // end sort
      } // end if
      const archean = document.getElementById("t753");
      const phanerozoic = document.getElementById("t751");
      const graph_start = archean.x.baseVal.value;
      const graph_end = (phanerozoic.x.baseVal.value + phanerozoic.width.baseVal.value);
      const graph_width = graph_end - graph_start;
      const marker_scale = graph_width / 4000;
      const marker_x = graph_end - (mya * marker_scale);
      markers[place_in_queue].x = marker_x;
      if (place_in_queue > 0) { previous_marker_x = markers[(place_in_queue - 1)].x; }
      const distance_from_last_marker = marker_x - previous_marker_x;
      if (highlight || (distance_from_last_marker >= 50)) {
        let line_pos = 160;
        if (highlight) { line_pos = 210; }
        markerGroup.append("svg:line")
            .attr("x1", marker_x)
            .attr("y1", 22)
            .attr("x2", marker_x)
            .attr("y2", line_pos)
            ...
            .style("stroke-width", () => {
              if (highlight) { return "2px"; }
              else { return "0.05em"; }
            });
        line_pos = line_pos + 10;
        const text_bits = text.split(" ");
        for (let i = 0; i < text_bits.length; i++) {
          markerGroup.append("text")
              .attr("x", marker_x)
              .attr("y", line_pos)
              .attr("id", "marker-text")
              .attr("class", "marker_text noselect")
              .style("text-anchor", "middle")
              .style("font-size", () => {
                if (highlight) { return "10px"; }
                else { return "8px"; }
              })
              .style("fill", () => {
                if (highlight) { return "black"; }
                else { return "black"; }
              })
              .text(text_bits[i]);
          line_pos = line_pos + 10;
        ...
        markerGroup.append("text")
            ...
            .text(mya + " mya");
      } // end if (highlight || (distance_from_last_marker >= 50))
    }, // end method
    //////////////////////////////////////////////////////////////////////
    // METHOD ////////////////////////////////////////////////////////////
    ...
    "redraw_markers": function() {
      ...
      // re-draw the markers
      for (let i = 0; i < markers.length; i++) {
        timescale.add_marker(markers[i].mya, markers[i].text, markers[i].highlight);
      } // end for loop
    }, // end method
    //////////////////////////////////////////////////////////////////////
    // METHOD ////////////////////////////////////////////////////////////
    "add_major_markers": function() {
      for (let i = 0; i < bio_intervals.length; i++) {
        timescale.add_marker(bio_intervals[i].mya, bio_intervals[i].event);
      } // end for loop
    } // end method
    //////////////////////////////////////////////////////////////////////
  }
